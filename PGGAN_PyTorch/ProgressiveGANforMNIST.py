...
class Discriminator( nn.Module ):
    ...
    def __init__(
        ...
        layers.append( nn.Conv2d( in_channels=n_fmaps, out_channels=n_rgb, kernel_size=1, stride=1, padding=0 ).to( self._device ) )
        layers.append( WScaleLayer( pre_layer = layers[-1] ).to( self._device ) )
        ...
        for epoch in tqdm( range(self._n_epoches), desc = "Epoches" ):
            ...
            for i, (images,targets) in enumerate( tqdm( dloader, desc = "minbatch process in DataLoader" ) ):
                ...
                if( iterations % n_sava_step == 0 ):
                    ...
                    save_image( tensor = images, filename = result_path + "/PGGAN_Image_epochs{}_iters{}.png".format( epoch, iterations ) )
