...
if __name__ == '__main__':
    ...
    for progress in range(progress_final):
        ...
        # バッチサイズの更新
        batch_size = args.batch_size[progress]
        del dloader_train
        dloader_train = torch.utils.data.DataLoader(Subset(ds_train, train_index), batch_size=batch_size, shuffle=True, num_workers = args.n_workers, pin_memory = True )
        ...
        for epoch in tqdm( range(n_epoche), desc = "epoches" ):
            for iter, inputs in enumerate( tqdm( dloader_train, desc = "progress={}, alpha={:0.4f}, epoch={}".format(progress, alpha, epoch) ) ):
                ...
                if inputs["latent_z"].shape[0] != batch_size:
                    break
                ...
                if( args.data_augument ):
                    image_t = DiffAugment(image_t, policy="color,translation")
