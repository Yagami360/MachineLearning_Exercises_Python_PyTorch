...
class Embeddings(nn.Module):
    ...
    """
    def __init__(self, config, image_height, image_width, in_channels=3):
        super(Embeddings, self).__init__()
        self.hybrid = None

        if config.patches.get("grid") is not None:
            grid_size = config.patches["grid"]
            patch_size = (image_height // 16 // grid_size[0], image_width // 16 // grid_size[1])
            n_patches = (image_height // 16) * (image_width // 16)
            self.hybrid = True
        else:
...
class Transformer(nn.Module):
    def __init__(self, config, image_height, image_width, vis):
        super(Transformer, self).__init__()
        self.embeddings = Embeddings(config, image_height=image_height, image_width=image_width)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, image_height=128, image_width=128, num_classes=2, zero_head=False, vis=False):
        super(VisionTransformer, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier

        self.transformer = Transformer(config, image_height, image_width, vis)
