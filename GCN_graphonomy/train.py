...
if __name__ == '__main__':
    ...
    parser.add_argument("--n_classes", type=int, default=20, help="クラス数")
    ...
    ds_train = CIHPDataset( args, args.dataset_dir, datamode = "train", data_augument = args.data_augument, debug = args.debug )
    ...
    # モデルの構造を定義する。
    #================================
    model = GraphConvolutionNetworks( n_inputs = 512, n_outputs = args.n_classes ).to(device)
    if( args.debug ):
        print( "model\n", model )
    ...
    for epoch in tqdm( range(args.n_epoches), desc = "epoches" ):
        for iter, inputs in enumerate( tqdm( dloader_train, desc = "minbatch iters" ) ):
            ...
            categories = inputs["categories"].to(device)
            categories_rev = inputs["categories_rev"].to(device)
            if( args.debug and n_print > 0):
                print( "image.shape : ", image.shape )
                print( "categories.shape : ", categories.shape )
                print( "categories_rev.shape : ", categories_rev.shape )
