...
from models.gcn import GraphConvolution, FeatureMaptoGraphProjection, InterGraphTransfer
...
class GraphonomyIntraGraphReasoning( nn.Module ):
    """
    Graphonomy の Intra-Graph Reasoning
    """
    def __init__( self, n_in_channels = 3, n_classes = 20 ):
        ...
        self.feature_to_graph = FeatureMaptoGraphProjection( in_features = 256, out_features = 128, n_nodes = n_classes )
    ...
    def forward(self, input, adj_matrix = None ):
        ...
        #------------------------------
        # backbone ネットワーク（ResNet）で projection（encode）
        ...
        #---------------------------------------------
        # Graphonomy / Intra-Graph Reasoning での処理
        #---------------------------------------------
        # DeepLab v3+ で enocder-decoder した特徴マップをグラフ構造に射影する
        graph = self.feature_to_graph(decode)
        print( "graph.shape : ", graph.shape )                              # torch.Size([2, 20, 128])

        # グラフ構造をグラフ畳み込み
        graph1 = self.graph_conv1(graph, adj_matrix)
        graph2 = self.graph_conv2(graph, adj_matrix)
        graph3 = self.graph_conv3(graph, adj_matrix)
        print( "graph1.shape : ", graph1.shape )              # torch.Size([adj_shape[0], 2, 20, 128])
...
class GraphonomyInterGraphTransfer( nn.Module ):
    """
    Graphonomy の Inter-Graph Transfer
    """
    def __init__( self, n_in_channels = 3, n_classes_source = 7, n_classes_target = 20 ):
        super(GraphonomyInterGraphTransfer, self).__init__()
        self.backbone = ResNet( block = Bottleneck, layers = [3, 4, 23, 3], n_in_channels = n_in_channels, output_stride = 16, BatchNorm = nn.BatchNorm2d, pretrained = True )
        self.aspp = ASPP( backbone = "resnet", output_stride = 16, BatchNorm = nn.BatchNorm2d, dropout = 0.0 )
        ...
        self.feature_to_graph = FeatureMaptoGraphProjection( in_features = 256, out_features = 128, n_nodes = n_classes_source )
    ...
    def forward(self, input, adj_matrix_source = None, adj_matrix_target = None, adj_matrix_transfer = None ):
        ...
        #--------------------------------------------
        # Graphonomy / Inter-Graph Transfer での処理
        #--------------------------------------------
        # DeepLab v3+ で enocder-decoder した特徴マップをグラフ構造に射影する
        graph = self.feature_to_graph(decode)
